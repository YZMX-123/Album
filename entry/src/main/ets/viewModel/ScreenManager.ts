...
export class ScreenManager {
  ...
  static getInstance(): ScreenManager {
    if (AppStorage.Get(EditConstants.APP_KEY_SCREEN_MANAGER) == null) {
      AppStorage.SetOrCreate(EditConstants.APP_KEY_SCREEN_MANAGER, new ScreenManager());
    }
    let manager: ScreenManager = AppStorage.Get(EditConstants.APP_KEY_SCREEN_MANAGER);
    return manager;
  }
  ...
  async initializationSize(win: window.Window): Promise<void> {
    ...
    AppStorage.SetOrCreate<boolean>(EditConstants.IS_FULL_SCREEN_KEY, properties.isFullScreen);
  ...
  setFullScreen(): void {
    ...
    try {
      topWindow.setLayoutFullScreen(true, () => {
        ...
        if (AppStorage.Get('deviceType') !== EditConstants.DEFAULT_DEVICE_TYPE) {
          this.hideStatusBar();
        } else {
  ...
  async setDefaultStatusBarProperties(): Promise<void> {
    ...
    try {
      topWindow.setSystemBarProperties(
        { statusBarColor: EditConstants.STATUS_BAR_BACKGROUND_COLOR,
          statusBarContentColor: EditConstants.STATUS_BAR_CONTENT_COLOR }, () => {
        Log.info(TAG, 'setStatusBarColor done');
      });
    } catch (err) {
